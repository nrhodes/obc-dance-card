...
| 10 | CSP has no `unsafe-eval`; `'unsafe-inline'` only on styles; report-only run showed no violations | `firebase/firebase.json` hosting `headers` (plan §14.1) | `grep -o "script-src[^;]*" firebase/firebase.json` shows no `unsafe-eval`, only `'self'` + the three Google/reCAPTCHA hosts; `grep -o "style-src[^;]*"` is the only header value containing `'unsafe-inline'`. Report-only run: deploy with a second `Content-Security-Policy-Report-Only` header (or use the browser devtools CSP debugger against the emulator/dev build) and click through every screen once; confirm the console shows zero CSP violations before removing the report-only header. | `code` (header content) + `pending` (report-only click-through — manual, do once before pilot) |
...
| 13 | Privacy statement published in-app; `eraseMember` tested | `eraseMember`: `firebase/functions/src/admin/members.ts`. Privacy statement page: **web** (owned by the concurrent web-track agent; not part of this backend pass) | Code: `npm run test:emu -w @obc/functions` → `src/admin/__tests__/members.emu.test.ts` covers `eraseMember` scrubbing `members`, `memberPrivate`, `visitors`, denormalised `entries`/`teams` names, and notifications, plus the Auth account deletion and the 30-day/active guards. Web: confirm a "Privacy" link/page exists in the app footer or Profile screen and its copy matches the NZ Privacy Act 2020 purpose statement (plan §8.1). | `code` (`eraseMember`) + `pending` (privacy statement page — web track) |
